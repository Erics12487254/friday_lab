...
class Order:
    ...
    def points(self):
        return int(self.total() // POINTS_DIVISOR) * self.customer.point_multiplier()

    def receipt(self):
        lines = []
        lines.append(f"Receipt for {self.customer.name} ({self.customer.tier})")
        lines.append("-" * 40)
...
def make_customer(name, tier):
    ...
    customer_cls = customer_classes.get(tier)
    if not customer_cls:
...
def make_orders():
    ...
    products = [create_product(name, price, cat) for name, price, cat in raw_products]
    ...
    for name, tier, item_data in old_orders:
