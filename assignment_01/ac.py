class AirConditioner:
    ...
    def __init__(self, brand, room_name, temperature=25, mode="cool", fan_speed=1):
        ...
        self.is_on = False
        self.temperature = temperature   #This directly changes the private attribute
        self.mode = mode
        self.fan_speed = fan_speed

    @property
    def is_energy_saving(self):
            return self.temperature >= 25   #_is_energy_saving stays whatever it was originally regardless of the temperature changes. 
    ...
    def temperature(self, value):
        ...
        self._is_energy_saving = value >= 25    #Inside the temperature setter, this calls the same setter again, which calls itself forever.
    ...
    def cooler(self):    #violating MIN_TEMP.
        if self.temperature > self.MIN_TEMP:
            self.temperature -= 1

    def warmer(self):    #violating MAX_TEMP.
    ...
    def __str__(self):
        ...
        return (f"{self.brand} AC in {self.room_name}: {power}, "
                f"{self.temperature}C, mode={self.mode}, fan={self.fan_speed}") #the bug is self.fan_speed which was previously self.fan. 
